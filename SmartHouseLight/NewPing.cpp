...
// Sets a new maximum distance for the timer
...
// Time that the ultrasonic wave takes to return to the sensor in microseconds
...
// Returns the measured distance in CMs
...
// Returns the median of given number of measured distances
unsigned long NewPing::ping_median(uint8_t it) {
	unsigned int last;

	// Reserve the array using malloc
	unsigned int* uS = (unsigned int *)malloc(sizeof(unsigned int) * it);
	...
	unsigned int ret_val = (uS[it >> 1]);
	// Release the reserved array
	free(uS);

	return ret_val; // Return the ping distance median. ( Element in the middle of the sorted array )
}
...
// Creates an event at a certain time ( in microseconds )
...
// Creates an event at a certain time ( in milliseconds )
...
// Converts echo time to a distance in centimeters
...
// Converts echo time to a distance in inches
